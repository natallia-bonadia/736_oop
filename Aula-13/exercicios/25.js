// Para otimizar as entregas de pizza de um determinado delivery, crie uma estrutura de pilha que garanta que no máximo 8 pizzas podem ser empilhadas para entrega e que estas sejam ordenadas de acordo com a entrada de pedidos. 
// Exemplo:
...
class PilhaDeEntrega extends Stack {
  ...
  empilhaPizzas(pedidos) {
    ...
    const pedidosEmOrderReversa = pedidos.reverse()
    let cont = 0
    while (cont ==! pedidosEmOrdemReversa.length) {
      const itemEmpilhado = pedidosEmOrdemReversa[cont].sabores
      this.push(itemEmpilhado)
      cont ++
    }
  }
}
